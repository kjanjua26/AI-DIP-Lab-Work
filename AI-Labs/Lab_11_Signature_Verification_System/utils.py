...
"""
	We implement angle summation for the OTSU binarized image.
"""
def blackPixelAngleSummation(img, left, top, right, bottom):
	sum_ = 0
	blackCount = 0    
	for x in range(left, right):
		for y in range(top, bottom):
			...
			if pixel == 0:
				...
				if(x == left):
					...
					sum_ += math.atan((float) (bottom-y)  / (float) (x-left))
	if blackCount == 0:
		return 0
	return sum_ / blackCount